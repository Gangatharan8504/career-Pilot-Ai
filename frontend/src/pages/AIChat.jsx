...
export default function AIChat({ userId }) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // Replace messages list with server history to keep sync
      if (data.history) {
        setMessages(data.history);
      } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '84vh' }}>
      ...
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className="glowing-title">Placement AI Chat</h1>
          <p style={{ color: 'var(--text-muted)' }}>
            Ask concepts (e.g. "Explain HashMap"), coding structure, resume improvements, or company preparation.
          </p>
        </div>
      </div>
        ...
        {messages.length === 0 ? (
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              <h3 style={{ color: 'white', marginBottom: '0.25rem' }}>Start your conversation</h3>
              <p style={{ fontSize: '0.9rem' }}>Type a placement topic below. For example: "Explain HashMap in Java"</p>
            </div>
          </div>
        ) : (
              ...
              <div key={idx} style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    background: isAI ? 'linear-gradient(135deg, var(--primary), var(--secondary))' : 'rgba(255, 255, 255, 0.05)',
                  ...
                  <div style={{
                    ...
                    textAlign: 'left',
                    whiteSpace: 'pre-wrap',
                    boxShadow: isAI ? 'none' : '0 4px 12px rgba(139, 92, 246, 0.2)'
                  }}>
                    {msg.text}
                  </div>
                </div>
      ...
      <form onSubmit={handleSend} style={{ display: 'flex', gap: '0.75rem' }}>
        <input
          type="text"
          placeholder="Ask something placement-related (e.g. 'What is sliding window method?')..."
